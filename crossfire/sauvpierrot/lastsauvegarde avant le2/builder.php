...
if(document.getElementById('high_RUS2_'+row).checked) { document.getElementById("photo_ru_slave2_"+row).src="pic/images/remot_hp.png"
	...
	var ActiveDeactiv = function ActiveDeactiv(id,row,id_tab){// on se sert de l'id du bouton pour changer sa couleur	
	    ...
	    if (bodyunit.className=='body_unit_inactive'){
			...
			if (T_unit[id_tab].type == "AU_Master"){// si cest un AU+Master on n'attend pas que l'input soit active
				...
				T_unit[id_tab].activated = "true";
				bodyunit.className='body_unit';
				document.getElementById(id_tab).checked = true;
			}else{	
			...
			if (document.getElementById(id_tab) !== null){document.getElementById(id_tab).checked = false;
			document.getElementById(id_tab).style.display = "none";
			}
			...
			for (var i = 0; i<cmpt_unit; i++){
				if (typeof elements[i] !== 'undefined'){
					...
					if (elements[i].getAttribute('row_number') == button_RowNumber){
						//alert('oui');
						if (elements[i].style.display == "none"){elements[i].style.display = "";	console.log('on a trouve le 1 er RU Slave');
	...
	function AddNewRU_Master(button_RowNumber){
		...
		if (i == cmpt_unit)	{AddNewRU(button_RowNumber);console.log('On ajoute New RU');}				
			draw_full_cable();
		}
	...
	function AddNewAU(button_RowNumber){
		...
		var new_AU=template_AU; 		// on recupe le template dajout de ligne
			...
			console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX RUS1 : ' + cmpt_unit);
			//alert(String(cmpt_unit));
			...
			console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX RUS2 : ' + cmpt_unit);			
		...
		var idElt = 'ROW_' + String(button_RowNumber); 
	       console.log('Insert at row : ' + idElt);
			...
			hide_AU_Slave(id_AUS1,id_AUS2);
			hide_RU_SLave(id_RUS1,id_RUS2);
			hide_RU_Master(id_RUM);
			ActiveDeactiv(0,num_nouv_row, id_AUM);
		}
		
	function hide_AU_Slave(id_AUS1,id_AUS2){
					//on recherche les bouton pour les cacher
			//on cache les slaves
			var elements_AU = document.getElementsByClassName('unit_slave');
			for (var i = 0; i<cmpt_unit; i++){
				if (typeof elements_AU[i] !== 'undefined'){
					console.log("element a cache ? " + elements_AU[i].id)
					if (elements_AU[i].id == id_AUS1){ //on hi
						elements_AU[i].style.display = "none";
					}
					if (elements_AU[i].id == id_AUS2){
						elements_AU[i].style.display = "none";
					}
				}
			}
	}
	function hide_RU_SLave(id_RUS1,id_RUS2){
		//SLAVE RU
			var elements_RU = document.getElementsByClassName('ru_unit_slave');
			for (var i = 0; i<cmpt_unit; i++){
				if (typeof elements_RU[i] !== 'undefined'){
					console.log("element a cache ? " + elements_RU[i].id)
					if (elements_RU[i].id == id_RUS1){ //on hi
						elements_RU[i].style.display = "none";
					}
					if (elements_RU[i].id == id_RUS2){
						elements_RU[i].style.display = "none";
					}
				}
			}
	}
	function hide_RU_Master(id_RUM){
					//MASTER RU
					console.log('Hide_RU_Master : ' + id_RUM);
			var elements_RUM = document.getElementsByClassName('ru_unit');
			for (var i = 0; i<cmpt_unit; i++){
				if (typeof elements_RUM[i] !== 'undefined'){
					console.log("element a cache ? " + elements_RUM[i].id)
					if (elements_RUM[i].id == id_RUM){ //on hi
						elements_RUM[i].style.display = "none";
					}
				}
			}
	}
	...
	<form method="post" id="form1" name="form" action="generate_graph.php" target="_blank">
		...
		<input type="button" class="button4" onclick="window.location='start.html';" value="Start Over"/> 	</form>
	</div>	
</div>
