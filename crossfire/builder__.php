				...
				if (typeof elements[i] !== 'undefined'){
					...
					if (elements[i].getAttribute('row_number') == button_RowNumber){
						...
						if (elements[i].style.display == "none"){
							...
							//change_module_AUS1(father,button_RowNumber);						
							//change_module_AUS2(father,button_RowNumber);//we dont know if slave 1 or 2 so we must call both function
							break;
						}
					}
				}
				...
				if (typeof elements[i] !== 'undefined'){
					...
					if (elements[i].getAttribute('row_number') == button_RowNumber){
						if (elements[i].style.display == "none"){
							...
							//on envoie le pere du pere ... le AUM du RUM
							//change_module_AUS1(T_unit[T_unit[elements[i].id].input].input,button_RowNumber);//on utilise father pour connaitre les module du RU
							//change_module_AUS2(T_unit[T_unit[elements[i].id].input].input,button_RowNumber);//on utilise father pour connaitre les module du RU
							//alert('yeha');
...
Choose_Pic_RUM(NewRU,new_row);
			...
			//setTimeout(function(){change_module_AUS1(NewRU,new_row)},1000);//on sait pas si cest un slave 1 ou 2 donc on appelle les 2 fonctions
			//	setTimeout(function(){change_module_AUS2(NewRU,new_row)},1000);
			//	setTimeout(function(){change_module_AUM(NewRU,new_row)},1000);
					...
					if (elements[i].getAttribute('row_number') == button_RowNumber){
						if (elements[i].style.display == "none"){
							...
							if (Save_Output(father,parseInt(elements[i].id))){//on a trouve le pere, on verifie quon puisse bien enregistrer l'unit comme une output du pere
								...
								ActiveDeactiv(0,button_RowNumber,elements[i].id);
								ip_activated(elements[i].id);
								//setTimeout(function(){change_module_AUM(father,button_RowNumber)},2000);//on envoie le pere de lunit
								//alert('iciiii');
								//alert('id : ' + elements[i].id + ' and row : ' + button_RowNumber);
								Choose_Pic_RUM(elements[i].id,button_RowNumber);
								//change_module_AUM(father,button_RowNumber);//on envoie le pere de lunit
